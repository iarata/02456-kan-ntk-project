...
def run_sweep():
    """Initialize wandb run with provided config"""
    with wandb.init() as run:
        ...
        wandb.log({
            ...
            "host_machine": wandb.run.host  # Log which machine is running this experiment
        })
        ...
        model_artifact = wandb.Artifact(
            ...
            type='model',
            description=f'KAN model from run {wandb.run.id}'
        )
        model_path = f'kan_model_{wandb.run.id}.pth'
...
def main():
    ...
    parser.add_argument('--mode', choices=['create', 'agent'], required=True,
                      help='create: create new sweep, agent: run sweep agent')
    parser.add_argument('--sweep_id', help='Sweep ID to join (required for agent mode)')
    parser.add_argument('--project', default='KANTK_MNIST', help='W&B project name')
    parser.add_argument('--entity', default=None, help='W&B entity/username')
    parser.add_argument('--count', type=int, default=None, 
                      help='Number of runs for this agent to execute')
    ...
    if args.mode == 'create':
        ...
        if not args.sweep_id:
            raise ValueError("sweep_id is required in agent mode")
